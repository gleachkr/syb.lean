...
class Among (β : Type u) (d : List (Type u)) where
  witness : Member β d

instance : Among α (α :: d) where
  witness := .head
...
def TermOf.ofSingleton {α} {d : List (Type u)}
  [Among α d] [Typeable α] (i : TermOf α [α]) : TermOf α d where
  witness := (Among.witness : Member α d)
  gmap F a :=
    i.gmap
      (fun {δ} [Typeable δ] [m : Among δ [α]] => by
...
instance (priority := 100) [Among α d] [Typeable α]
...
| nil  : Dict full []
| cons {α : Type u} [Among α full] :
    TermOf α full → Dict full d → Dict full (α :: d)

/-- Lookup a `TermOf` by membership witness in a `Dict`. -/
def Dict.lookup {full : List (Type u)} {d : List (Type u)}
  (xs : Dict full d) : Member β d → TermOf β full
| .head   => by
  cases xs with
